...
class SE_Block(nn.Module):
    def __init__(self, out_channels, ratio=16): # an input convolutional block and the current number of channels it has
        ...
        self.avgpool = nn.GlobalAveragePooling2D()      # 1 x 1 x C
        self.dense1  = nn.Linear(out_channels//ratio)   # 1 x 1 x C/r
        self.relu    = nn.ReLU(True)                    # 1 x 1 x C/r
        self.dense2  = nn.Linear(out_channels)          # 1 x 1 x C
        self.sigmoid = nn.Sigmoid()                     # 1 x 1 x C
